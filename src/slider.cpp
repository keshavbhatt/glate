...
Slider::Slider(QWidget *parent) : QWidget(parent), ui(new Ui::Slider) {
   ...
   *  currentIndex = index where the slider will start, this updates with time
   * set in timeout progeressbar = show slide timer based progressbar on top.
  ...
  connect(ui->next, &QAbstractButton::clicked, this, &Slider::next);
  connect(ui->prev, &QAbstractButton::clicked, this, &Slider::prev);
  connect(ui->skip, &QAbstractButton::clicked, this, &QWidget::close);

  timer = new QTimer(this);
  timer->setInterval(duration);
  connect(timer, &QTimer::timeout, this, &Slider::next);

  loadSlides();
}

Slider::~Slider() { delete ui; }
...
void Slider::createSlide(QString html, int index) {
  ...
  slide->hide();
  slide->setText(html);
  connect(slide, &Slide::linkActivated, this, [=](QString link) {
    if (link == "start") {
      this->close();
    } else {
  ...
  navBtn->setStyleSheet("border-radius:" +
                        QString::number(navBtn->width() / 2));
