...
TranslationDownloader::TranslationDownloader(QObject *parent, QList<QUrl> urls,
...
void TranslationDownloader::start() {
  ...
  emit downloadStarted(
      "<span style='color:green;'>Share: </span>downloading part: " +
      QString::number(current + 1) + " of " + QString::number(total));
}
          ...
          [=](QNetworkReply *rep) {
            if (rep->error() == QNetworkReply::NoError) {
              emit downloadFinished(
                  "Downloaded part: " + QString::number(current + 1) + " of " +
