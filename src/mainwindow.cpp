    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(_player,&QMediaPlayer::mediaStatusChanged,[=](QMediaPlayer::MediaStatus mediastate){
        QPushButton *playBtn = this->findChild<QPushButton*>(_player->objectName().split("_").last());
        if(playBtn != nullptr && (mediastate == QMediaPlayer::LoadingMedia||
                mediastate == QMediaPlayer::BufferingMedia)){
            playBtn->setIcon(QIcon(":/icons/loader-2-fill.png"));
        }
        if(playBtn != nullptr && (mediastate == QMediaPlayer::BufferedMedia) ){
            playBtn->setIcon(QIcon(":/icons/stop-line.png"));
        }
    });
    ...
    int minWid = ui->counter->fontMetrics().boundingRect(ui->counter->text()).width();
    ...
    _share->setWindowTitle(QApplication::applicationName()+" | Share translation");
...
void MainWindow::init_settings()
{
    ...
    connect(settingsWidget,&Settings::translationRequest,[=](const QString selected){
...
void MainWindow::init_textOptionWidget()
{
    textOptionWidget = new QWidget(0);
    textOptionForm.setupUi(textOptionWidget);
    textOptionWidget->setWindowFlags(Qt::Popup);

    textOptionWidget->move(QCursor::pos());
...
void MainWindow::textSelectionChanged(QTextEdit *editor)
{
    ...
    if(!selection.isEmpty()){
        ...
        connect(textOptionForm.copyPushButton,&QPushButton::clicked,[=](){
...
void MainWindow::resizeFix(){
    ...
    ui->left_buttons_widget->resize(ui->right_button_widget->width(),ui->left_buttons_widget->height());
}
...
{
    int x = ui->translationWidget->geometry().center().x()-_translate->width()/2;
    int y = ui->translationWidget->geometry().center().y()-(_translate->height()+_translate->height()/2);
    _translate->move(x,y);
...
void MainWindow::translate_clicked()
{
    ...
    if(_request==nullptr){
        ...
        QString urlStr = "https://translate.googleapis.com/translate_a/single?client=gtx&sl="+
                ...
                src1;
        QString url = QUrl(urlStr).toString(QUrl::EncodeSpaces);
        if(url.isEmpty()){
    ...
    foreach (const QJsonValue &val, jsonArray) {
        if(val.isArray() && !val.toArray().isEmpty()) {
            foreach (const QJsonValue &val2, val.toArray()) {
                if(val2.isArray() && !val2.toArray().isEmpty()&& val2.toArray().count()>=2) {
...
{
    if(message.contains("host",Qt::CaseInsensitive)|| message.contains("translate.googleapis.com")){
...
void MainWindow::on_play2_clicked()
{
    ...
    if(_player->state() != QMediaPlayer::PlayingState){
        if(_supportedTts.contains(getTransLang(),Qt::CaseInsensitive)==false){
            showError("Selected language '"+getTransLang()+"' is not supported by TTS Engine.\nPlease choose different Language.");
            return;
        }
...
void MainWindow::on_play1_clicked()
{
    ...
    if(_player->state() != QMediaPlayer::PlayingState){
        if(_supportedTts.contains(getSourceLang(),Qt::CaseInsensitive)==false){
            showError("Selected language '"+getSourceLang()+"' is not supported by TTS Engine.\nPlease choose different Language.");
            return;
        }
...
void MainWindow::on_settings_clicked()
{
    if(settingsWidget->isVisible()==false){
        ...
        QScreen* pScreen = QGuiApplication::screenAt(this->mapToGlobal({settingsWidget->width()/2,0}));
        QRect availableScreenSize = pScreen->availableGeometry();
        settingsWidget->move(availableScreenSize.center()-settingsWidget->rect().center());
...
void MainWindow::on_history_clicked()
{
    if(historyWidget->isVisible()==false){
        ...
        QScreen* pScreen = QGuiApplication::screenAt(this->mapToGlobal({historyWidget->width()/2,0}));
        QRect availableScreenSize = pScreen->availableGeometry();
