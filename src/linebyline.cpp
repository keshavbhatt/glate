...
LineByLine::LineByLine(QWidget *parent, QClipboard *clipborad) :
    QWidget(parent),
    ui(new Ui::LineByLine)
{
    ui->setupUi(this);
    this->clipborad = clipborad;
    connect(ui->src1->verticalScrollBar(),&QScrollBar::valueChanged,[=](int val){
       ui->src2->verticalScrollBar()->setValue(val);
    });
    connect(ui->src2->verticalScrollBar(),&QScrollBar::valueChanged,[=](int val){
       ui->src1->verticalScrollBar()->setValue(val);
    });
}
...
void LineByLine::setData(QList<QStringList> currentTranslationData,QString sLang,QString tLang,QString tId){
    translationId = tId;
